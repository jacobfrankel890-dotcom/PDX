...
export function ProfileCompanyPicker({ value, onChange }: Props) {
    ...
    <>
      <Pressable style={styles.trigger} onPress={() => setOpen(true)}>
        <View style={styles.triggerText}>
          <Text style={styles.triggerLabel}>Default company</Text>
      ...
      <Modal visible={open} transparent animationType="slide" onRequestClose={() => setOpen(false)}>
        ...
        <View style={[styles.sheet, { paddingBottom: insets.bottom + spacing.md }]}>
          <Text style={styles.sheetTitle}>Default company</Text>
          <ScrollView>
            {COMPANIES.map((c) => {
...
export function companyToRegion(company: CompanyValue): PdxRegion {
  return company;
}
